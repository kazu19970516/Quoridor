...
function eval(b1,b2,w1,w2,turn,blue,yellow){
  ...
  //return make_root(n,b2,b1,node_wall)
...
function miniMax(b1,b2,w1,w2,turn,blue,yellow,alpha,beta,level){
  ...
  if(level == SEARCH_LEVEL){
    //操作を返す
    return [type,bestX,bestY,value]
  }else{
    return value;
  }
}

function minimax_work(board,wall,b1,b2,w1,w2,turn,blue_wall,yellow_wall){
  node_board = JSON.parse(JSON.stringify(board));
  node_wall = JSON.parse(JSON.stringify(wall));
  blue = blue_wall
  yellow = yellow_wall
  //黄色がcpuのとき
  return miniMax(b1,b2,w1,w2,turn,blue,yellow,-9999,9999,SEARCH_LEVEL);
}
